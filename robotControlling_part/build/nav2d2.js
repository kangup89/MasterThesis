 ...
 */
NAV2D2.ImageMapClientNav = function(options) {
  var that = this;
  options = options || {};
  ...
  client.on('change', function() {
    that.navigator = new NAV2D2.Navigator({
      ros : that.ros,
 ...
 *   * withOrientation (optional) - if the Navigator should consider the robot orientation (default: false)
 */
NAV2D2.Navigator = function(options) {
  var that = this;
  options = options || {};
  var ros = options.ros;
  ...
  poseListener.subscribe(function(msg) {
    ...
    //console.log("RobotPose! : " + pose.position.x + "   " + pose.position.y);
 ...
 *   * viewer - the main viewer to render to
 */
NAV2D2.OccupancyGridClientNav = function(options) {
  var that = this;
  options = options || {};
  this.ros = options.ros;
  ...
  client.on('change', function() {
    ...
    //that.viewer.shift(-2, -3);
    //console.log("Scale! " + client.currentGrid.pose.position.x + "  " + client.currentGrid.pose.position.y);
  });
};

NAV2D2.CostMapClientNav = function(options) {
  var that = this;
  options = options || {};
  this.ros = options.ros;
  var topic = options.topic || '/map';
  ...
  // setup a client to get the map
  var client2 = new ROS2D2.CostMapGridClient({
    ros : this.ros,
    rootObject : this.rootObject,
    continuous : continuous,
    topic : topic,
  ...
  client2.on('change', function() {
    ...
    if(baseClient != null){
      //that.viewer.scaleToDimensions(baseClient.currentGrid.width, baseClient.currentGrid.height);
      //that.viewer.shift(baseClient.currentGrid.pose.position.x, baseClient.currentGrid.pose.position.y);
    }
    // scale the viewer to fit the map
...
NAV2D2.CostMapClientNav = function(options) {
  ...
  client2.on('change', function() {
    ...
    if(NAV2D.base_width != null){
      //console.log("not null!" + NAV2D.base_width);
      that.viewer.scaleToDimensions(NAV2D.base_width, NAV2D.base_height);
      that.viewer.shift(NAV2D.base_x, NAV2D.base_y);
    }else{
      //console.log("null!");
    }
    // scale the viewer to fit the map
