 ...
 */
ROS2D2.ImageMap = function(options) {
  options = options || {};
...
ROS2D2.ImageMap.prototype.__proto__ = createjs.Bitmap.prototype;
 ...
 */
ROS2D2.ImageMapClient = function(options) {
  var that = this;
  options = options || {};
  var ros = options.ros;
...
ROS2D2.ImageMapClient.prototype.__proto__ = EventEmitter2.prototype;
 ...
 */
ROS2D2.Viewer = function(options) {
  var that = this;
  options = options || {};
 ...
 */
ROS2D2.Viewer.prototype.addObject = function(object) {
 ...
 */
ROS2D2.Viewer.prototype.scaleToDimensions = function(width, height) {
  this.scene.x = 0; this.scene.y = this.height; this.scene.scaleX = 1; this.scene.scaleY = 1;
 ...
 */
ROS2D2.Viewer.prototype.shift = function(x, y) {
  //////console.log(x + "   " + y + "  " + this.scene.x + "   " + this.scene.y);
 ...
 */
ROS2D2.OccupancyGrid = function(options) {
  options = options || {};
  var message = options.message;
...
ROS2D2.OccupancyGrid.prototype.__proto__ = createjs.Bitmap.prototype;

ROS2D2.OccupancyGrid.prototype.costmap = function(message) {
  ////////console.log("grid costmap");
 ...
 */
ROS2D2.OccupancyGridClient = function(options) {
  var that = this;
  options = options || {};
  var ros = options.ros;
  var topic = options.topic || '/map';
  var costmap = options.costmap;
  ...
  // work-around for a bug in easeljs -- needs a second object to render correctly
  this.rootObject.addChild(new ROS2D2.Grid({size:1}));
  ...
  rosTopic.subscribe(function(message) {
    ...
    that.currentGrid = new ROS2D2.OccupancyGrid({
      message : message
    });
    ...
    /*var rosTopic2 = new ROSLIB.Topic({
      ros : ros,
      name : costmap,
      messageType : 'nav_msgs/OccupancyGrid',
      compression : 'png'
    ...
    rosTopic2.subscribe(function(message) {
      ////console.log("costmap sub!");
      that.currentGrid.costmap(message);
    })*/
...
ROS2D2.OccupancyGridClient.prototype.__proto__ = EventEmitter2.prototype;
 ...
 *   * rootObject (optional) - the root object to add this marker to
 */
ROS2D2.OccupancyGridSrvClient = function(options) {
  var that = this;
  options = options || {};
  var ros = options.ros;
...
ROS2D2.OccupancyGridSrvClient.prototype.__proto__ = EventEmitter2.prototype;
 ...
 */
ROS2D2.ArrowShape = function(options) {
	var that = this;
	options = options || {};
...
ROS2D2.ArrowShape.prototype.__proto__ = createjs.Shape.prototype;
...
ROS2D2.Grid.prototype.__proto__ = createjs.Shape.prototype;
 ...
 *   * pulse (optional) - if the marker should "pulse" over time
 */
ROS2D2.NavigationArrow = function(options) {
  var that = this;
  options = options || {};
  var size = options.size || 10;
  var strokeSize = options.strokeSize || 3;
...
ROS2D2.NavigationArrow.prototype.__proto__ = createjs.Shape.prototype;
 ...
 *   * pulse (optional) - if the marker should "pulse" over time
 */
ROS2D2.NavigationImage = function(options) {
  var that = this;
  options = options || {};
  var size = options.size || 10;
...
ROS2D2.NavigationImage.prototype.__proto__ = createjs.Bitmap.prototype;
 ...
 *   * strokeColor (optional) - the createjs color for the stroke
 */
ROS2D2.PathShape = function(options) {
	options = options || {};
 ...
 */
ROS2D2.PathShape.prototype.setPath = function(path) {
	this.graphics.clear();
	if (path !== null && typeof path !== 'undefined') {
		this.graphics.setStrokeStyle(this.strokeSize);
		this.graphics.beginStroke(this.strokeColor);
...
ROS2D2.PathShape.prototype.__proto__ = createjs.Shape.prototype;
 ...
 */
ROS2D2.PolygonMarker = function(options) {
//	var that = this;
	options = options || {};
 ...
 * Internal use only
 */
ROS2D2.PolygonMarker.prototype.createLineShape = function(startPoint, endPoint) {
 ...
 * Internal use only
 */
ROS2D2.PolygonMarker.prototype.editLineShape = function(line, startPoint, endPoint) {
	line.graphics.clear();
 ...
 * Internal use only
 */
ROS2D2.PolygonMarker.prototype.createPointShape = function(pos) {
 ...
 */
ROS2D2.PolygonMarker.prototype.addPoint = function(pos) {
 ...
 */
ROS2D2.PolygonMarker.prototype.remPoint = function(obj) {
	var index;
//	var point;
 ...
 * @param position of type ROSLIB.Vector3
 */
ROS2D2.PolygonMarker.prototype.movePoint = function(obj, newPos) {
	var index;
	var point;
	if (obj instanceof createjs.Shape) {
		index = this.pointContainer.getChildIndex(obj);
 ...
 */
ROS2D2.PolygonMarker.prototype.splitLine = function(obj) {
	var index;
	var line;
	if (obj instanceof createjs.Shape) {
 ...
 * Internal use only
 */
ROS2D2.PolygonMarker.prototype.drawFill = function() {
	var numPoints = this.pointContainer.getNumChildren();
	if (numPoints > 2) {
...
ROS2D2.PolygonMarker.prototype.__proto__ = createjs.Container.prototype;
 ...
 */
ROS2D2.TraceShape = function(options) {
//	var that = this;
	options = options || {};
 ...
 */
ROS2D2.TraceShape.prototype.addPose = function(pose) {
 ...
 */
ROS2D2.TraceShape.prototype.popFront = function() {
...
ROS2D2.TraceShape.prototype.__proto__ = createjs.Shape.prototype;
 ...
 */
ROS2D2.PanView = function(options) {
	options = options || {};
...
ROS2D2.PanView.prototype.startPan = function(startX, startY) {
	this.startPos.x = startX;
	this.startPos.y = startY;
};

ROS2D2.PanView.prototype.pan = function(curX, curY) {
 ...
 */
ROS2D2.ZoomView = function(options) {
	options = options || {};
	this.rootObject = options.rootObject;
...
ROS2D2.ZoomView.prototype.startZoom = function(centerX, centerY) {
...
ROS2D2.ZoomView.prototype.zoom = function(zoom) {
...
ROS2D2.CostMapViewer = function(options) {
  var that = this;
  options = options || {};
  var divID = options.divID;
  this.width = options.width;
  ...
  document.getElementById(divID).appendChild(canvas2);
  // create the easel to use
  this.scene = new createjs.Stage(canvas2);
 ...
 * @param object - the object to add
 */
ROS2D2.CostMapViewer.prototype.addObject = function(object) {
  this.scene.addChild(object);
};
 ...
 * @param height - the height to scale to in meters
 */
ROS2D2.CostMapViewer.prototype.scaleToDimensions = function(width, height) {
  this.scene.x = 0; this.scene.y = this.height; this.scene.scaleX = 1; this.scene.scaleY = 1;
  // store the actual offset in the ROS coordinate system
  var tmpY = this.height - (this.scene.y * this.scene.scaleY);
  this.scene.scaleX = this.width / width;
 ...
 * @param y - the amount to shift by in the y direction in meters
 */
ROS2D2.CostMapViewer.prototype.shift = function(x, y) {
  ////console.log(x + "   " + y + "  " + this.scene.x + "   " + this.scene.y);
  this.scene.x -= (x * this.scene.scaleX);
  this.scene.y += (y * this.scene.scaleY);
  ////console.log(x + "   " + y + "  " + this.scene.x + "   " + this.scene.y);
 ...
 *   * message - the occupancy grid message
 */
ROS2D2.CostMapGrid = function(options) {
  options = options || {};
  var message = options.message;
  ...
  var imageData = context2.createImageData(this.width, this.height);
  for ( var row = 0; row < this.height; row++) {
    for ( var col = 0; col < this.width; col++) {
      // determine the index into the map data
      var mapI = col + ((this.height - row - 1) * this.width);
...
ROS2D2.CostMapGrid.prototype.__proto__ = createjs.Bitmap.prototype;

ROS2D2.CostMapGrid.prototype.costmap = function(message) {
  ////console.log("grid costmap");
 ...
 *   * continuous (optional) - if the map should be continuously loaded (e.g., for SLAM)
 */
ROS2D2.CostMapGridClient = function(options) {
  var that = this;
  options = options || {};
  var ros = options.ros;
  var topic = options.topic || '/map';
  ...
  rosTopic.subscribe(function(message) {
    ...
    that.currentGrid = new ROS2D2.CostMapGrid({
      message : message
    });
...
ROS2D2.CostMapGridClient.prototype.__proto__ = EventEmitter2.prototype;
