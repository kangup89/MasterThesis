;(function(window, QB, app, CONFIG, $, Backbone) {
    ...
    $(function() {
        ...
        var Router = Backbone.Router.extend({
            ...
            'dashboard': function() {
                ...
                //fucntion(users){
                    ...
                    console.log("users 0 name: ", users[0].full_name);
                    console.log(users.length);
                    var active = 0;
                    for(var i = 0; i < users.length; i++){
                        if(users[i].full_name == 'kangupRobot'){
                            var user = users[i];
                            app.callees[user.id] = user.name;
                            console.log("callee id: ", user.id);
                            console.log("callee name: ", user.full_name);
                            active = 1;
                        }
                    }
         ...
         * INIT
         */
        //var CREDS = app.helpers.getQueryVar('creds') === 'test' ? CONFIG.CREDENTIALS.test :
        //CONFIG.CREDENTIALS.prod;
        var CREDS = app.helpers.getQueryVar('creds') === 'test' ? CONFIG.CREDENTIALS.test :
        CONFIG.CREDENTIALS.prod;
        ...
        /*CREDS.appId = 48361;
        CREDS.authKey = 'bBB2kGxPYfSyPmC';
        CREDS.authSecret = 'rwhECCDuNgdDgKX';*/
        ...
        $(document).on('submit','.j-join', function() {
            ...
            data.username = 'HMT_application_kangup';
            data.room = 'kangupDVPIS';
            ...
            app.helpers.join(data).then(function (user) {
                ...
                console.log('log id: ', app.caller.id);
        ...
        QB.webrtc.onRemoteStreamListener = function onRemoteStreamListener(session, userId, stream) {
            ...
            console.log('stream1: ', session, userId, stream);
